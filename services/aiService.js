...
// FIX: Added "answer" field to the prompt format so Show Answer actually works
const generateQuestions = async (text) => {
  try {
    const raw = await groqCall(`Generate 10 exam practice questions with model answers.
Return ONLY a JSON array, no markdown, no explanation.
Format: [{"question":"...","type":"short_answer","hint":"...","answer":"..."}]
The "answer" field must contain a complete model answer (2-4 sentences).
Notes: ${text.slice(0, 4000)}`);
    const parsed = extractJSON(raw, 'array');
    if (Array.isArray(parsed) && parsed.length > 0) return parsed;
    return [{ question: 'Summarise the main points', type: 'short_answer', hint: 'Check notes', answer: 'Review your notes for key concepts.' }];
  } catch (err) {
